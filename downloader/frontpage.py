...
def updateFrontPageData(db, geek):
    import time
    start = time.time()
    fpg = FrontPageGeek(geek)
    getFrontPagePlaysData(db, fpg)
    fpg.save(db)
    end = time.time()
...
class FrontPageGeek(object):
    ...
    def toMap(self):
        result = {}
        for key in ["geek", "totalPlays", "distinctGames", "top50", "sdj", "owned", "want", "wish", "trade",
                    "prevOwned", "friendless", "utilisation", "cfm", "tens", "zeros", "ext100", "mv", "hindex"]:
            result[key] = self.__dict__[key]
        return result
        
    def save(self, db):
        db.saveRow(self.toMap(), "frontpagegeek", "geek = '%s'" % self.geek, False)
        
def getFrontPagePlaysData(db, fpg):
    ...
    for (series, count) in data:
        if series == "Spiel des Jahre":
            ...
            fpg.mv = count
    getFrontPageFriendlessMetricData(db, fpg)    
    getFrontPageHindexData(db, fpg)
...
def getFrontPageFriendlessMetricData(db, fpg):
