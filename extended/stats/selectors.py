...
class PublishedYearSelector(Selector):
    key = "pub"
    arity = 1
    def __init__(self, year):
        import datetime
        ...
        if self.year == 0:
            self.year = datetime.date.today().year
        Selector.__init__(self, "Published in %d" % self.year, PublishedYearSelector.key)
...
class LastPlayedInYearSelector(Selector):
    ...
    def __init__(self, year):
        ...
        if self.year == 0:
            import datetime
            self.year = datetime.date.today().year
        Selector.__init__(self, "Last Played in %d" % self.year, LastPlayedInYearSelector.key)
...
SELECTOR_CLASSES = [ AllGamesSelector, OwnedGamesSelector, PreviouslyOwnedGamesSelector, RatedGamesSelector, PlayedGamesSelector,
                    ...
                    BookSelector, GeeklistSelector, GameSelector, PublishedYearSelector ]
